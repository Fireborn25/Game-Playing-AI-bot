...
int evaluate(const Board &b)
{
    ...
    if (b.data.b_pawn_ws == DEAD){
        ...
        val += 2*(getx(b.data.b_pawn_ws)-3);
        val += 2*min(gety(b.data.b_pawn_ws),abs(gety(b.data.b_pawn_ws) - 1));
    }
    if (b.data.w_pawn_ws == DEAD){
        val -= 100;
    }
    else{
        val -= 2*(4-getx(b.data.w_pawn_ws));
        val -= 2*min(abs(gety(b.data.w_pawn_ws -5)),abs(gety(b.data.w_pawn_ws) - 6));
    }
    if (b.data.b_pawn_bs == DEAD){
        val += 100;
    }
    else{
        val += 2*(getx(b.data.b_pawn_bs)-3);
        val += 2*min(gety(b.data.b_pawn_bs),abs(gety(b.data.b_pawn_bs) - 1));
    }
    if (b.data.w_pawn_bs == DEAD){
        val -= 100;
    }
    else{
        val -= 2*(4-getx(b.data.b_pawn_ws));
        val -= 2*min(abs(gety(b.data.b_pawn_ws -5)),abs(gety(b.data.b_pawn_ws) - 6));
    }
    return val*mul;
}
...
int minimax(Board &b, int depth, int alpha, int beta, bool maximizingPlayer, int check)
{
    ...
    if (legalMoves.size() == 0){
        if ((maximizingPlayer and c) or (!maximizingPlayer and !c)){
            return INT_MIN;
        }
        return INT_MAX;
    }
